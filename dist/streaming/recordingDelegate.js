...
class RecordingDelegate {
     ...
     */
    async updateRecordingConfiguration(configuration) {
    ...
    async *handleRecordingStreamRequest(streamId) {
        ...
        try {
            ...
            for await (const fragmentBuffer of fragmentGenerator) {
                ...
                this.log.debug(`[HKSV] Fragment #${fragmentCount}, size: ${fragmentBuffer.length}B, total: ${totalBytes}B`, this.cameraName);
                yield {
                    data: fragmentBuffer,
                    isLast: false,
                };
            }
            this.log.info(`[HKSV] Recording stream ${streamId} completed. Fragments: ${fragmentCount}, bytes: ${totalBytes}`, this.cameraName);
        }
        catch (error) {
    ...
    closeRecordingStream(streamId, reason) {
        ...
        switch (reason) {
            ...
            case 5:
                ...
                break;
            case 6:
                this.log.warn('❌ [HKSV] Recording ended due to STREAM FORMAT INCOMPATIBILITY - Check H.264 parameters!', this.cameraName);
                break;
            case 7:
    ...
    async startPreBuffer() {
        ...
        if (this.videoConfig.prebuffer) {
            if (!this.preBuffer) {
                this.preBuffer = new prebuffer_1.PreBuffer(this.log, this.videoConfig.source || '', this.cameraName, this.videoProcessor);
                if (!this.preBufferSession) {
    ...
    async *handleFragmentsRequests(configuration, streamId) {
        ...
        if (this.videoConfig.prebuffer && this.preBuffer) {
            ...
            const input = await this.preBuffer.getVideo(prebufferLength);
     ...
     */
    startFFMPegFragmentedMP4Session(ffmpegPath, ffmpegInput, videoOutputArgs) {
        return new Promise((resolve, reject) => {
            const args = ['-hide_banner', ...ffmpegInput];
            ...
            args.push('-f', 'mp4', ...videoOutputArgs, '-movflags', 'frag_keyframe+empty_moov+default_base_moof+omit_tfhd_offset', 'pipe:1');
            // Terminate any previous process
            if (this.process && !this.process.killed) {
                this.process.kill('SIGKILL');
            }
            this.process = (0, child_process_1.spawn)(ffmpegPath, args, {
                env: process.env,
            ...
            async function* generator() {
                ...
                while (true) {
                    try {
                        const header = await (0, hksvHelpers_1.readLength)(cp.stdout, 8);
                        const length = header.readInt32BE(0) - 8;
                        const type = header.slice(4).toString();
                        if (length < 0 || length > 50 * 1024 * 1024) {
                            throw new Error(`Invalid MP4 box: ${length}B for ${type}`);
                        }
                        const data = await (0, hksvHelpers_1.readLength)(cp.stdout, length);
                        yield { header, length, type, data };
                    }
                    catch (error) {
                        if (!processKilledIntentionally)
                            throw error;
                        break;
                    }
                }
